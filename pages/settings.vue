<template>
  <div
    class="mx-auto w-full max-w-6xl px-4 py-8 pb-28 sm:px-6 lg:px-8 space-y-8 relative"
  >
    <div
    ...
    >
      <div>
        ...
        <p class="mt-2 text-sm text-slate-400">
          Manage your organization, team members and personal preferences.
        </p>
      </div>

      <div
        v-if="currentOrg"
        class="flex items-center gap-3 bg-slate-800/50 px-4 py-2.5 rounded-xl ring-1 ring-slate-700/50 backdrop-blur-sm"
      >
        <div
          class="flex h-8 w-8 items-center justify-center rounded-full bg-slate-900 ring-1 ring-slate-700 text-slate-400"
        >
          <Icon
            name="lucide:shield"
            size="14"
            v-if="['Owner', 'Admin'].includes(myOrgRole)"
          />
          <Icon name="lucide:user" size="14" v-else />
        </div>
        <div class="flex flex-col justify-center">
          <span
            class="text-[10px] font-medium text-slate-500 uppercase tracking-wider leading-none mb-1"
    ...
    <div class="flex flex-col md:flex-row gap-8 items-start">
      <aside
        class="w-full md:w-64 shrink-0 flex flex-wrap md:flex-col gap-2 md:gap-0 md:space-y-1"
      >
        <button
        ...
        >
          <Icon
            :name="tab.icon"
            size="18"
            :class="activeTab === tab.id ? 'text-purple-400' : ''"
          />
      ...
      <main class="flex-1 w-full min-w-0 space-y-6">
        ...
        >
          <div
            class="rounded-2xl bg-slate-800/40 p-6 sm:p-8 ring-1 ring-slate-700/50 backdrop-blur-sm relative overflow-hidden"
          >
            <div
              class="absolute right-0 top-0 h-64 w-64 -translate-y-1/2 translate-x-1/2 rounded-full bg-purple-500/10 blur-[80px] pointer-events-none"
            ></div>

            <div
              class="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-700/50 pb-6 mb-6 relative z-10 gap-4"
            >
              <div class="flex items-center gap-4">
                <img
                  src="/img/illustrations/1.png"
                  alt="Company Logo"
                  class="w-16 h-16 rounded-xl object-cover bg-slate-900 ring-1 ring-slate-700 shadow-lg"
                />
                <div>
            ...
            <div
              class="mt-8 flex justify-start border-t border-slate-700/50 pt-6 relative z-10"
            >
              <TSupportTriggerD
                variant="ghost"
        ...
        <section
          v-if="activeTab === 'team' && can('view_team')"
          class="animate-in fade-in duration-300 space-y-6"
        >
          <div
          ...
          <div v-if="isLoading" class="p-12 text-center text-slate-400">
            <Icon
              ...
              class="animate-spin mx-auto h-8 w-8 text-purple-500 mb-2"
            />
          ...
          <div
            v-else
            class="rounded-2xl bg-slate-800/40 ring-1 ring-slate-700/50 backdrop-blur-sm"
          >
            <div class="divide-y divide-slate-700/50">
              ...
              <div
                ...
                :key="member.id"
                class="flex items-center justify-between p-5 hover:bg-slate-800/60 transition-colors first:rounded-t-2xl last:rounded-b-2xl"
              >
                <div class="flex items-center gap-4">
                  <img
                    v-if="member.avatar"
                  ...
                  <div
                    v-else
                    class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-slate-700 ring-1 ring-slate-600 text-white font-bold uppercase"
                  >
                  ...
                  <div>
                    <div class="flex items-center gap-2">
                      <p
                        class="text-sm font-bold text-white max-w-[150px] sm:max-w-xs truncate"
                      >
                      ...
                      <span
                        ...
                        class="rounded bg-blue-500/10 px-1.5 py-0.5 text-[10px] font-bold text-blue-400 ring-1 ring-inset ring-blue-500/20"
                      >
                        You
                      </span>
                    ...
                    <div
                      class="flex items-center gap-2 text-xs text-slate-400 mt-1"
                    >
                ...
                >
                  ...
                  <div class="relative menu-container">
                    ...
                    >
                      <button
                        @click="handlePasswordReset(member)"
          ...
          >
            <div
              class="relative w-full max-w-md rounded-2xl bg-slate-900 border border-slate-800 p-6 shadow-xl"
            >
              <h3 class="text-lg font-bold text-white mb-2">
                Add New Team Member
              </h3>
              <p class="text-xs text-slate-400 mb-6">
                Creates account immediately. They will access the workspace
                using your organization's shared default credentials.
              </p>
              ...
              <form @submit.prevent="handleAddUser" class="space-y-4">
                ...
                <div>
                  <label
                    class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1"
                    >Role Type <span class="text-rose-500">*</span></label
                  >
                  <select
          ...
          >
            <div
              class="relative w-full max-w-sm rounded-2xl bg-slate-900 border border-slate-800 p-6 shadow-xl text-center"
            >
              <div
                class="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-rose-500/10 mb-4 ring-8 ring-slate-900"
              >
              ...
              <p class="text-sm text-slate-400 mb-6 leading-relaxed">
                ...
                from this workspace? They will immediately lose access to all
            ...
            >
              ...
              <div v-if="!tempDefaultPassword" class="space-y-4">
                <p class="text-sm text-slate-300">
                  Are you sure you want to alter security authorization and
                  reset credentials for
                  <strong>{{ selectedResetUser?.name }}</strong>
                </p>
              ...
              <div v-else class="space-y-4">
                <div
                  class="rounded-lg bg-emerald-500/10 border border-emerald-500/20 p-4 text-emerald-400 text-sm flex gap-3 items-start"
                >
                  <Icon
                  ...
                  />
                  <div>
                    <span class="font-bold">Password Overridden!</span> Please
                    manually copy and relay this temporary fallback credential
                    securely to the user.
                  </div>
                </div>

                <div
                  class="bg-slate-950 rounded-xl p-3.5 border border-slate-800 text-center select-all"
                >
                  <span
                    class="text-[10px] text-slate-500 block uppercase font-mono tracking-wider mb-1"
                    >Temporary System Password</span
                  >
                  <span
                    class="text-lg font-mono text-cyan-400 font-bold tracking-wide"
                    >{{ tempDefaultPassword }}</span
                  >
                </div>
        ...
        >
          <div
            class="rounded-2xl bg-slate-800/40 p-6 sm:p-8 ring-1 ring-slate-700/50 backdrop-blur-sm"
          >
        ...
        <section
          v-if="activeTab === 'billing' && can('view_billing')"
...
const manager = computed(() => {
  ...
  if (relationshipManagerDetails.value) {
...
const handleAddUser = async () => {
  ...
  if (success) {
    ...
    showNotification({
      title: 'Error',
      message:
        'Failed to add team member. Please verify your permissions and try again.',
      type: 'error',
    })
  }
}
...
const handlePasswordReset = (member: any) => {
  triggerPasswordReset(member.userId, `${member.firstName} ${member.lastName}`)
  activeMenuId.value = null
}

const handleRoleUpdate = (member: any) => {
...
const passwordMessage = ref<{ type: 'success' | 'error'; text: string } | null>(
  null,
)
