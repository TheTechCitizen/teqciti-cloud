<template>
  <div
    class="mx-auto w-full max-w-5xl px-4 py-8 pb-28 sm:px-6 lg:px-8 relative min-h-screen"
  >
    <!-- Header -->
    <div
    ...
    >
      <div>
        ...
        <p class="mt-2 text-sm text-slate-400">
          Complete your deliverables to keep projects healthy and optimum
        </p>
      </div>
    ...
    >
      <div
        class="flex h-16 w-16 items-center justify-center rounded-full bg-rose-500/10 ring-1 ring-rose-500/30 mb-4"
      >
      ...
      <p class="text-slate-400 text-sm mt-1 max-w-sm">
        Something went wrong fetching your action items. Check your connection
        and try again.
      </p>
      <button
        @click="refreshActions"
        class="mt-5 inline-flex items-center gap-2 rounded-xl bg-purple-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-purple-500 transition-colors"
      >
    ...
    <div v-else class="space-y-8">
      ...
      <div
        class="relative overflow-hidden rounded-3xl bg-slate-800 ring-1 ring-slate-700/50 flex flex-col md:flex-row items-center p-5 sm:p-8 gap-5 md:gap-8 shadow-xl"
      >
        <div
          class="absolute -left-20 -top-20 h-64 w-64 rounded-full bg-emerald-500/10 blur-[80px] pointer-events-none"
        ></div>
        <div
          class="absolute -right-20 -bottom-20 h-64 w-64 rounded-full bg-purple-500/10 blur-[80px] pointer-events-none"
        ></div>

        <div
          class="relative shrink-0 w-full md:w-auto flex justify-center md:block mb-2 md:mb-0"
        >
          <div class="relative">
            <div
              class="absolute inset-0 bg-emerald-500/20 blur-2xl rounded-full"
            ></div>
            <div
              class="relative z-10 flex h-20 w-20 md:h-28 md:w-28 items-center justify-center rounded-full bg-slate-900 ring-1 ring-emerald-500/50 shadow-2xl"
            >
              <Icon
                name="lucide:check-circle"
                class="text-emerald-400 w-10 h-10 md:w-12 md:h-12"
              />
            </div>
            <div
              class="absolute -bottom-2 left-1/2 -translate-x-1/2 z-20 whitespace-nowrap rounded-full bg-gradient-to-r from-emerald-500 to-teal-500 px-2.5 py-0.5 md:px-3 md:py-1 text-[10px] md:text-xs font-bold text-white shadow-lg"
            >
              Weekly Sprint
            </div>
          </div>
        ...
        <div class="flex-1 w-full z-10">
          <div
            class="flex flex-wrap items-end justify-between gap-4 mb-3 md:mb-2"
          >
            <div>
              ...
              <p
                class="text-3xl md:text-4xl font-black text-white mt-0.5 md:mt-1"
              >
                {{ completionRate
                }}<span class="text-lg md:text-xl text-emerald-500 ml-1"
                  >%</span
                >
              </p>
            </div>
            <div class="text-left md:text-right">
              ...
              <p
                class="text-sm font-bold text-emerald-400 flex items-center md:justify-end gap-1 mt-0.5 md:mt-1"
              >
                <Icon name="lucide:activity" size="14" />
                {{ sprintCompletedTasks.length }} of
          ...
          <div
            class="h-2.5 md:h-3 w-full overflow-hidden rounded-full bg-slate-900 ring-1 ring-inset ring-slate-700/50 relative"
          >
            <div
          ...
          <p
            class="text-[11px] md:text-xs font-medium text-slate-500 mt-2.5 md:mt-3 text-left md:text-right"
          >
            Resets every Monday
          </p>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
          <div class="space-y-6">
            ...
            >
              ...
              <div
                class="flex gap-2 p-1 rounded-full bg-slate-900/50 ring-1 ring-slate-700/50"
              >
                <button
            ...
            <div
              v-if="
                activeFilter === 'pending' && sprintPendingTasks.length === 0
              "
              class="text-center py-16 rounded-2xl bg-slate-800/30 ring-1 ring-slate-700/50"
            >
              <div
                class="inline-flex h-20 w-20 items-center justify-center rounded-full bg-emerald-500/10 mb-4 ring-1 ring-emerald-500/20"
              >
                <Icon
              ...
              <p class="text-slate-400 text-sm mt-1">
                You have no pending tasks for this week's sprint.
              </p>
            </div>
          ...
          <div
            v-if="!isLastWeekOfMonth && monthlyOutlookTasks.length > 0"
          ...
          >
            ...
            >
              <h2
                class="text-base font-bold text-slate-300 flex items-center gap-2"
              >
                <Icon
              ...
              <span class="text-xs text-slate-500"
                >Optional — get a head start any time</span
              >
            </div>
        ...
        <div class="space-y-6">
          <h2
            class="text-lg font-bold text-white flex items-center gap-2 border-b border-slate-700/50 pb-4"
          >
          ...
          <div
            ...
            class="rounded-2xl bg-slate-800/30 p-6 ring-1 ring-slate-700/50 text-center text-sm text-slate-400"
          >
          ...
          <div
            v-else
            class="rounded-2xl bg-slate-800/30 p-5 ring-1 ring-slate-700/50"
          >
            <div class="relative pl-6 border-l border-slate-700 space-y-8 py-2">
              ...
              >
                ...
                <div
                  :class="[
                    'absolute -left-[31px] flex h-4 w-4 items-center justify-center rounded-full ring-4 ring-slate-800/30',
                    getEventColor(event.type).dotBg,
                  ]"
                >
                  <div
                    :class="[
                      'h-2 w-2 rounded-full',
                      getEventColor(event.type).innerBg,
                    ]"
                  ></div>
                </div>
    ...
    >
      ...
      <div
        class="animate-in zoom-in slide-in-from-bottom-10 fade-in duration-500 flex flex-col items-center bg-slate-900 p-8 rounded-3xl ring-1 ring-emerald-500/30 shadow-2xl relative z-10"
      >
        <div
          class="flex h-20 w-20 items-center justify-center rounded-full bg-emerald-500/20 ring-1 ring-emerald-400/50 mb-4"
        >
        ...
        <p class="text-slate-400 text-sm mt-2 text-center">
          Your weekly progress has been updated.
        </p>
      </div>
    </div>
  </div>
...
const {
  ...
  error,
  refreshActions,
} = await useActionCenter(orgId.value)
...
const sprintPendingTasks = computed(() =>
  sprintTasks.value.filter((t) => t.computedStatus === 'pending'),
)
const sprintCompletedTasks = computed(() =>
  sprintTasks.value.filter((t) => t.computedStatus === 'completed'),
)
const visibleSprintTasks = computed(() =>
  activeFilter.value === 'pending'
    ? sprintPendingTasks.value
    : sprintCompletedTasks.value,
)

const monthlyOutlookTasks = computed(() =>
  tasks.value.filter((t) => !t.inSprint && t.computedStatus === 'pending'),
)
...
const handleManualRefresh = async () => {
  ...
  const taskCountBefore =
    sprintPendingTasks.value.length + monthlyOutlookTasks.value.length
  ...
  if (newTasks > 0 && newEvents > 0) {
    show({
      title: 'New activity',
      message: `${newTasks} new task${newTasks === 1 ? '' : 's'} and ${newEvents} new event${newEvents === 1 ? '' : 's'}.`,
      type: 'info',
      showInTray: true,
    })
  } else if (newTasks > 0) {
    show({
      ...
      message: `${newTasks} new task${newTasks === 1 ? '' : 's'} just came in.`,
      type: 'info',
      showInTray: true,
    })
  } else if (newEvents > 0) {
    show({
      ...
      message: `${newEvents} new event${newEvents === 1 ? '' : 's'} on your schedule.`,
      type: 'info',
      showInTray: true,
    })
  } else {
    show({
      ...
      message: 'No new tasks or events right now.',
      type: 'success',
      showInTray: false,
    })
  }
